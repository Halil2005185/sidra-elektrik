...
function AdminSiber({ activePage, setActivePage }) {
    const navItems = [
        {
            id: "dashboard",
            label: "Dashboard",
        ...
        <aside className="w-64 min-h-screen bg-gradient-to-b from-slate-900 to-slate-950 flex flex-col shadow-2xl flex-shrink-0">
            ...
            <nav className="flex-1 py-4 px-3 space-y-1">
                ...
                {navItems.map(({ id, label, icon }) => (
                    ...
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 flex-shrink-0 ${activePage === id ? "text-sky-400" : "text-slate-500 group-hover:text-slate-300"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
                            {icon}
                        </svg>
                        {label}
                        {activePage === id && (
