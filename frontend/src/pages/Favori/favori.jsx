            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-12">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-rose-50 rounded-full mb-4 border border-rose-100">
                        ...
                        <span className="text-xs font-semibold text-rose-500 tracking-wider uppercase">Favorilerim</span>
                    </div>
                    <h2 className="text-3xl md:text-4xl font-bold text-[#2D2418] mb-3">
                        Favori{" "}
                        <span className="bg-gradient-to-r from-rose-500 to-pink-500 bg-clip-text text-transparent">Ürünlerim</span>
                    </h2>
                    <p className="text-[#8C7B6B] text-sm">
                        {products.length > 0 ? `${products.length} ürün favorilerinizde` : "Henüz favori ürününüz yok"}
                    </p>
                </div>
                ...
                {products.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-24">
                        ...
                        <h3 className="text-xl font-bold text-[#3B2F1E] mb-2">Favori ürün bulunamadı</h3>
                        <p className="text-[#B8A88A] text-sm mb-6">Beğendiğiniz ürünleri favorilere ekleyin</p>
                        <a href="/" className="px-6 py-3 bg-gradient-to-r from-[#C49A3C] to-[#D4A84B] text-white font-semibold rounded-full shadow-lg shadow-[#C49A3C]/20 hover:shadow-xl transition-all duration-300">
                            Ürünleri Keşfet
                        </a>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {products.map((product, index) => (
                            <CardItem key={product.id} product={product} index={index} />
                        ))}
                    </div>
                )}
            </div>
